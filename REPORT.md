...
- 틀린 5건 — 로그의 판정 이유와 질문 원문을 읽었다

| 문항 | 정답 → 예측 | 질문 요지 | 원인 |
|---|---|---|---|
| 400002 | USAGE → FEEDBACK | 1차 번역 때 왜 글자 수를 안 맞추게 해 놨냐 | 불만 표현에 끌림 — 08에서 줄였지만 10부터 거의 매번 남음 |
| 400018 | USAGE → FEEDBACK | 화면 캡션이 전부 확인 표시로 떠서 너무하지 않냐 | 위와 같음 |
| 400084 | STYLE_RULE → QC_EXPLAIN | 읽기 속도가 계속 넘치는데 아웃점을 얼마까지 늘려도 되나 | "읽기 속도가 넘친다"가 검사 결과 신호로 읽힘. 08에서 한 번, 11 이후 거의 매번 |
| 400105 | FEEDBACK → USAGE | 한글 폴더에서 exe가 꺼진다 | 문서에 없는 증상인데 원인을 묻는 말투 — 아래 '구조적 한계' |
| 400146 | OUT_OF_SCOPE → USAGE | Subtitle Edit 자체의 파형 표시 메뉴 | 13 상태 4회 중 이 회차에서만 틀림 — 흔들림 범위. 사용법 근거를 SE 플러그인 화면 기준으로 다시 쓴 것과 관련 있을 수 있다(추정) |
