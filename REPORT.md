...
| 문서 | 카테고리 | 조각 | 출처 |
|---|---|---|---|
| `docs/10_usage.md` | 사용법 | 16 | 생성기 README (옵션·동작을 코드와 대조) |
...
| 카테고리 | 정의 | 처리 방식 | 기대 도구 |
|---|---|---|---|
| `USAGE` 사용법 | 생성기를 **어떻게 쓰는가** — 옵션·실행·리포트 형식·결과 파일 | 문서 조회 | `search_usage` |
| `QC_EXPLAIN` 검사 결과 설명 | 검사가 낸 **특정 항목**(규칙 번호)의 뜻·근거·고치는 법 | 문서 조회 | `get_check_info` |
...
|---|---|---|---|
| `USAGE` | `U-01`~`U-16` | `10_usage.md` | 목차(ID·제목)만 — 본문은 도구로 조회 |
| `QC_EXPLAIN` | `Q-00`, `Q-C01`~`Q-S13` + 항목이 인용한 조항 | `20_qc_checks.md` (+ `30`) | 목차만 |
...
| 경계 | 판정 |
|---|---|
| "미구현 검사 표시가 뭐냐" vs "S02가 떴는데" | 형식 질문은 사용법, 특정 항목은 검사 결과 |
...
- 예: "SDH 검사에서 S02 읽기 속도 초과, 기준이?" → `must` "SDH 성인 초당 최대 14자" / `forbid` "성인 기준이 12자라고 말한다"
...
- 주의
  - 실험 10 뒤에 근거 문서 두 곳(C10·S13 설명)과 준비된 Q&A를 고쳤다. **그 뒤로는 API 사용 제한(사용자 결정) 때문에 다시 재지 않았다** — 위 수치는 실험 10 시점이다
...
flowchart TD
    ...
    answer --> guard
    guard{"guard<br/>수치·옵션·조각 인용·URL·접수번호<br/>근거에 있나?"}
    guard -- 통과 --> END2([답변])
...
- **검증은 기계적으로** — 답변 속 수치·옵션 이름·조각 인용·URL·접수번호가 조회한 조각(원문 발췌 포함)에 있는지. 서술의 뜻은 못 본다 → 채점기가 본다
...
- **API를 쓰지 않는 시험으로 화면 흐름을 확인** — `tests/test_offline.py`가 모델 호출을 막고(막혔는지부터 확인) 가짜 응답으로 29개 시험
